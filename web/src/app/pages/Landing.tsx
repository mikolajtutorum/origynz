        ...
        <div className="o-shell relative pb-16 pt-16 sm:pb-24 sm:pt-24">
          <div className="mx-auto max-w-3xl text-center">
            <p className="mx-auto inline-flex items-center gap-2 rounded-full border border-accent-edge bg-accent-soft px-3.5 py-1.5 text-[12px] font-medium text-accent">
              ...
              {t('Origynz — Family Storytelling Platform')}
            </p>
            <h1 className="o-display mt-6 text-[2.6rem] leading-[1.05] sm:text-7xl">
              {t('Where family stories')}
              <br />
              <span className="bg-gradient-to-r from-emerald-700 via-emerald-600 to-copper-600 dark:from-emerald-300 dark:via-emerald-200 dark:to-copper-300 bg-clip-text text-transparent">{t('take root.')}</span>
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-ink-soft">
              {t('Build living family trees, preserve photos and records, and map every relationship — private by default, collaborative when you choose.')}
            </p>
            <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
              <Link to="/register" className="o-btn-primary min-h-12 px-7 text-base">
                {t('Start your tree — free')}
              </Link>
              <Link to="/login" className="o-btn-secondary min-h-12 px-7 text-base">
                {t('Log in')}
              </Link>
            </div>
            <p className="mt-4 text-[13px] text-ink-muted">{t('GEDCOM import supported — bring your existing research with you.')}</p>
          </div>
            ...
            <div className="o-card relative overflow-hidden p-2 sm:p-3">
              <div className="o-dotgrid relative rounded-xl border border-edge-faint bg-inset px-4 py-10 sm:py-14">
                <div className="mx-auto flex max-w-xl flex-col items-center gap-6">
                  ...
                  {/* Illustrative sample names above are intentionally not translated. */}
                  ...
                  <PreviewCard initials="MW" name="Margaret Whitfield" dates={t('b. 1948 · 4 children')} focus />
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="border-t border-line py-16 sm:py-20">
        <div className="o-shell">
          <div className="mx-auto max-w-2xl text-center">
            <p className="o-eyebrow">{t('Everything in one workspace')}</p>
            <h2 className="o-display mt-3 text-3xl sm:text-4xl">{t('Serious genealogy, made welcoming')}</h2>
          </div>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <FeatureCard
              title={t('Living family trees')}
              copy={t('Interactive tree charts with marriages, branches, and focus views — built to handle thousands of relatives smoothly.')}
              icon={
            ...
            <FeatureCard
              copper
              title={t('GEDCOM import & export')}
              copy={t('Bring decades of research from any genealogy tool, photos included — and take your data with you anytime.')}
              icon={
                <svg className="h-5.5 w-5.5" viewBox="0 0 24 24" {...stroke}>
            ...
            />
            <FeatureCard
              title={t('Rich person profiles')}
              copy={t('Names, dates, places, photos, and notes on every relative — with living people protected by default.')}
              icon={
                <svg className="h-5.5 w-5.5" viewBox="0 0 24 24" {...stroke}>
            ...
            <FeatureCard
              copper
              title={t('Photos & media')}
              copy={t('A family photo library linked to the people in your tree, with portraits on every card.')}
              icon={
                <svg className="h-5.5 w-5.5" viewBox="0 0 24 24" {...stroke}>
            ...
            />
            <FeatureCard
              title={t('Relationship paths')}
              copy={t('Ask how any two people are connected and follow the chain of parents, spouses, and children between them.')}
              icon={
                <svg className="h-5.5 w-5.5" viewBox="0 0 24 24" {...stroke}>
            ...
            <FeatureCard
              copper
              title={t('Fast, modern workspace')}
              copy={t('A responsive app that works beautifully on your phone at a family gathering or on a big screen at your desk.')}
              icon={
                <svg className="h-5.5 w-5.5" viewBox="0 0 24 24" {...stroke}>
      ...
      <section className="o-shell py-16 sm:py-24">
        <div className="o-dotgrid relative overflow-hidden rounded-[2rem] border border-accent-edge bg-inset px-6 py-14 text-center sm:px-12">
          ...
          <div className="relative">
            ...
            <h2 className="o-display mx-auto mt-3 max-w-2xl text-3xl sm:text-4xl">{t('Your family history belongs to your family.')}</h2>
            <p className="mx-auto mt-4 max-w-xl text-[15px] leading-7 text-ink-soft">
              {t('Trees are private unless you share them. Living relatives are protected, exports are always available, and there are no ads — ever.')}
            </p>
      ...
      <footer className="border-t border-line">
        <div className="o-shell flex flex-col gap-3 py-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-[13px] text-ink-muted">
            ...
            {apiUp ? t('Connected to the Origynz API') : t('Connecting to the Origynz API…')}
          </p>
        </div>
      </footer>
    </main>
