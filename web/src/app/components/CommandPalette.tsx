      ...
      >
        <div className="flex items-center gap-3 border-b border-edge px-4">
          ...
          <input
            ...
            placeholder={t('Search people, or jump to a page…')}
        ...
        <div className="max-h-[50vh] overflow-y-auto p-2">
          {pages.length > 0 && (
            <>
              <p className="px-3 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-muted/80">{t('Go to')}</p>
              {pages.map((p) => {
                flatIndex += 1;
                const i = flatIndex;
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 font-medium">{t(p.label)}</span>
                    <span className="text-xs text-ink-muted">{t(p.hint)}</span>
                  </button>
                );
              })}
            </>
          ...
          {people.length > 0 && (
            <>
              <p className="px-3 pb-1 pt-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-muted/80">{t('People')}</p>
              {people.map((p) => {
                flatIndex += 1;
                const i = flatIndex;
                return (
          ...
          {items.length === 0 && (
            <p className="px-3 py-8 text-center text-sm text-ink-muted">
              {t('No matches. Try a person’s name or a page like “photos”.')}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 border-t border-edge px-4 py-2.5 text-[11px] text-ink-muted">
          <span className="flex items-center gap-1.5">
            <span className="o-kbd">↑↓</span> {t('navigate')}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="o-kbd">↵</span> {t('open')}
          </span>
        </div>
      </div>
    </div>
